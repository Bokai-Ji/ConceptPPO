...
from common.misc_util import adjust_lr, adjust_lr_concept, get_n_params
import torch
...
class PPO(BaseAgent):
    ...
    def train(self, num_timesteps, type_policy):
        ...
        while self.t < num_timesteps:
            ...
            self.logger.dump()
            if type_policy == 'easy-200':
                self.optimizer = adjust_lr(self.optimizer, self.learning_rate, self.t, num_timesteps) #num_timesteps
            elif type_policy == 'concept-easy-200':
                self.optimizer = adjust_lr_concept(self.optimizer, self.learning_rate, self.t)
            # Save the model
