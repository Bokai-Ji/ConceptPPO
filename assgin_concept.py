...
for path in data_list:
    ...
    for idx,frame in enumerate(episode):
        ...
        scaled_img = cv2.resize(img, (64*ZOOM_SCALE,64*ZOOM_SCALE))
        for idx_,blob in enumerate(blob_list):
            center = blob['center']
            scaled_img[center[0]*ZOOM_SCALE][center[1]*ZOOM_SCALE] = np.array([255,255,255])
            cv2.putText(scaled_img, str(idx_), (center[1]*ZOOM_SCALE, center[0]*ZOOM_SCALE), cv2.FONT_HERSHEY_SIMPLEX, FONT_SIZE, (255, 0, 0), FONT_THICKNESS)
        ...
        while entity_idx < len(frame['entities']):
            ...
            try:
                ...
                if color not in colors:
                    ...
                    plt.pause(0.01)
                    plt.clf()
                    print(f"[INFO] Entity {entity_id} with color {color} added.")
                    entity_type = input(f"[INFO] Assign an entity type: ")
...
with open(PATH+'entities.pkl', "wb") as f:
    pickle.dump(list(zip(colors, types)), f)
    f.close()
    print("Episode done.")